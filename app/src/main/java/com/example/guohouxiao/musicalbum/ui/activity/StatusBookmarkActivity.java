...
import com.example.guohouxiao.musicalbum.adapter.StatusBookmarkAdapter;
...
public class StatusBookmarkActivity extends BaseActivity {
    ...
    private StatusBookmarkAdapter mAdapter;
    private List<StatusBookmark> mList = new ArrayList<>();

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_status_bookmark);
            ...
            public void done(List<AVStatus> list, AVException e) {
                if (list.size() != 0) {
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if (((String) data.get("type")).equals("StatusBookmark")) {
                            StatusBookmark statusBookmark = new StatusBookmark();
                            statusBookmark.setUserId((String) data.get(Config.USERID));
                            statusBookmark.setNickname((String) data.get(Config.NICKNAME));
                            statusBookmark.setAlbumId((String) data.get("album_id"));
                            statusBookmark.setAlbumname((String) data.get(Config.ALBUMNAME));
                            mList.add(statusBookmark);
                            mAdapter = new StatusBookmarkAdapter(StatusBookmarkActivity.this, mList);
